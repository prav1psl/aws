...
public class AWSQueue {
    ...
    public AWSQueue(String queueEndpoint) {
        this.queueEndpoint = queueEndpoint;
    }

    static public AWSQueue createQueue(String name) throws Exception {
        ...
        AWSQueue newQueue = new AWSQueue(cqr.getCreateQueueResult().getQueueUrl());
        System.out.println("Created a new queue.  Queue url: " + newQueue.getQueueEndpoint());
        return newQueue;
    }

    static public List<AWSQueue> listQueues(String prefix) throws Exception {
        SQSClient sqsClient = new SQSClient(SampleDriver.QueueServiceURL);
        ...
        List<AWSQueue> queues = new ArrayList<AWSQueue>();
        if (lqr.getListQueuesResult() != null && lqr.getListQueuesResult().getQueueUrl() != null) {
            for (String queueUrl : lqr.getListQueuesResult().getQueueUrl()) {
                queues.add(new AWSQueue(queueUrl));
            }
        }
        return queues;
    }
